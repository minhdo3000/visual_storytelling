...
import time
import numpy as np
from .model_utils import AttentionLayer, luong_gate_attention, VisualEncoder, _smallest
...
class BaseModel(nn.Module):
    ...
    def init_hidden(self, batch_size, bi, dim):
        ...
        if self.rnn_type == 'gru' or self.rnn_type == 'gru_att':
    ...
    def forward(self, features, caption):
        ...
        for i in range(self.seq_length):
            cap_att_state = None
            if self.is_attn:
                self.attention.init_context(out_e.transpose(0, 1))
                cap_att_state, _ = self.attention(state_d.squeeze())
            log_probs, state_d = self.decode(out_e, last_word, state_d, caption=cap_att_state)
    ...
    def sample(self, features, sample_max, rl_training=False, pad=False):
        ...
        for t in range(self.seq_length):
            ...
            if self.is_attn:
                self.attention.init_context(out_e.transpose(0, 1))
                cap_att_state, _ = self.attention(state_d.squeeze())
            log_probs, state_d = self.decode(out_e, last_word, state_d, caption=cap_att_state, penalize_previous=True)
    ...
    def topK(self, features, beam_size=5):
        ...
        for k in range(batch_size):
            ...
            cap_att_state = None
            if self.is_attn:
                self.attention.init_context(out_e_k.transpose(0, 1))
                cap_att_state, _ = self.attention(state_d_k.squeeze())
            log_probs, state_d_k = self.decode(out_e_k, last_word, state_d_k, caption=cap_att_state, penalize_previous=True)
    ...
    def predict(self, features, beam_size=5):
        ...
        for k in range(batch_size):
            ...
            if self.is_attn:
                self.attention.init_context(out_e_k.transpose(0, 1))
                cap_att_state, _ = self.attention(state_d_k.squeeze())
            log_probs, state_d_k = self.decode(out_e_k, last_word, state_d_k, caption=cap_att_state,
                                               penalize_previous=True)
            log_probs[:, 1] = log_probs[:, 1] - 1000  # never produce <UNK> token
            neg_log_probs = -log_probs
